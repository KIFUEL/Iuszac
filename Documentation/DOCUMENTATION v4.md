...
| **Enrutamiento** | `go_router` | Navegación por ShellRoute con barra de navegación en móvil y NavigationRail en pantallas ≥800px. Guards de ruta según `user_type`. |
...
| `user_type` | `TEXT` | **`user` / `mentor` / `admin`** — controla permisos de la aplicación. Def: `user`. |
...
| --- | --- | --- |
| `id` | `UUID` PK | Identificador único. |
| `mentor_id` | `UUID` FK | Referencia al perfil con `user_type = mentor`. |
...
| `session_date` | `TIMESTAMP` | Fecha y hora programada de la sesión. |
| `expires_at` | `TIMESTAMP` | Fecha límite de vigencia; sesiones pasadas no se muestran. |
| `created_at` | `TIMESTAMP` | Fecha de creación del registro. |

> **Nota RLS:** Solo perfiles con `user_type = mentor` o `admin` pueden hacer `INSERT` en esta tabla. Un mentor solo puede `UPDATE` o `DELETE` sus propias sesiones y únicamente si `session_date > NOW()`.
...
│  └────────────────────────────────────┘  │
│                                    [ + ] │
│  FAB visible solo si user_type=mentor    │
└──────────────────────────────────────────┘
```

#### Notas de Implementación
*   **Pestaña "Todas":** Consume `mentorshipSessionsProvider` filtrando `expires_at > NOW()`. El buscador filtra por `specialty`.
*   **Pestaña "Mías":** Solo visible si `currentUser.user_type == 'mentor'`. Consume `myMentorshipSessionsProvider(mentorId)`. Muestra sesiones expiradas también (con badge "VENCIDA") para historial.
*   **Pestaña "Me inscribí":** Consume `enrolledSessionsProvider`. Muestra sesiones inscritas por el usuario autenticado.
*   **FAB "+":** Solo visible si `currentUser.user_type == 'mentor'`. Navega a `/mentorship/new`.
...
*   **Editar/Eliminar:** En la pestaña "Mías", cada tarjeta muestra un menú (⋮) con opciones Editar y Eliminar, solo si `session_date > NOW()`.
...
| Badge VENCIDA | Chip gris | No | Si `session_date < NOW()` (solo en pestaña "Mías"). |
| Fecha de sesión | Texto con ícono | Sí | `session_date` formateada `dd/MM/yyyy HH:mm`. |
...
| Menú editar/eliminar | IconButton (⋮) | No | Solo en tarjetas propias del mentor, solo si la sesión no ha pasado. |
| FAB Nueva sesión | FloatingActionButton | No | Solo visible para `user_type == 'mentor'`. |
...
| Fecha y Hora | IconRow | Sí | `session_date` formateada. |
...
| Fecha de la Sesión | DateTimePicker | Sí | Fecha y hora de impartición (`session_date`). No permite fechas pasadas. |
| Fecha Límite (Vigencia) | DatePicker | Sí | `expires_at` — date después de la cual no se muestra la sesión. Por defecto = `session_date`. |
...
#### Descripción General
Tablero de control exclusivo para perfiles con `user_type = 'admin'`. Centraliza la gestión de contenido, la moderación y la administración de usuarios mentor.
...
#### Notas de Implementación
*   GoRouter bloquea el acceso si `currentUser.user_type != 'admin'` y redirige a `/`.
*   La sección de moderación muestra posts y comentarios reportados (o recientes) para revisión y eliminación.
*   La gestión de mentores permite buscar usuarios y cambiar su `user_type` de `user` a `mentor` o viceversa.

#### Interfaz y Validaciones
| **Campo / Elemento** | **Tipo** | **Requerido** | **Detalles / Validación** |
| --- | --- | --- | --- |
| Guard de ruta admin | GoRouter redirect | Sí | Redirige a `/` si `user_type != 'admin'`. |
...
#### Notas de Implementación
*   El administrador puede cambiar `user_type` entre `user` y `mentor`. El cambio a `admin` no está disponible desde esta pantalla (se hace directamente en Supabase Dashboard por seguridad).
*   El UPDATE de `user_type` solo es posible mediante una política RLS que permita a los `admin` modificar ese campo específico, o mediante una función SQL con `SECURITY DEFINER`.
...
| Lista Mentores Actuales | ListView | Sí | Usuarios con `user_type = 'mentor'`. Botón "Revocar Mentor". |
...
| Botón Revocar Mentor | Botón rojo outlined | Sí | UPDATE `user_type = 'user'` en `profiles`. |
| Botón Hacer Mentor | Botón verde | Sí | UPDATE `user_type = 'mentor'` en `profiles`. |
...
| --- | --- | --- |
| `/mentorship/new` | `user_type == 'mentor'` | `/mentorship` |
| `/mentorship/edit/:id` | `user_type == 'mentor'` AND es dueño AND `session_date > NOW()` | `/mentorship` |
| `/admin` y sub-rutas | `user_type == 'admin'` | `/` |
...
| `mentorUsersProvider` | `FutureProvider` | Perfiles con `user_type = 'mentor'` (solo admin). |
...
-- Solo mentor puede insertar mentorship_sessions
CREATE POLICY "mentor_insert_sessions"
ON mentorship_sessions FOR INSERT
WITH CHECK (
  auth.uid() = mentor_id AND
  (SELECT user_type FROM profiles WHERE id = auth.uid()) = 'mentor'
);
...
────────────────────────────────────────────────────────────────────────────────
*IusZac · Documentación de Pantallas v4.1 · Zacatecas, Junio 2026*
