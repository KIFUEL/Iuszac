...
DROP POLICY IF EXISTS "Solo administradores pueden actualizar actualizaciones" ON public.legal_updates;
CREATE POLICY "Solo administradores pueden actualizar actualizaciones" ON public.legal_updates 
  FOR UPDATE USING (
    EXISTS (
      SELECT 1 FROM public.profiles 
      WHERE profiles.id = auth.uid() AND profiles.role = 'admin'
...
DROP POLICY IF EXISTS "Solo administradores pueden eliminar actualizaciones" ON public.legal_updates;
CREATE POLICY "Solo administradores pueden eliminar actualizaciones" ON public.legal_updates 
  FOR DELETE USING (
    EXISTS (
      SELECT 1 FROM public.profiles 
      WHERE profiles.id = auth.uid() AND profiles.role = 'admin'
